...
class Cal_Impl:
    ...
    def __init__(self, enabled):
        ...
        self.start_cal = Calendar(fone,
                                  selectmode="day",
                                  mindate=tw_start_date,
                                  maxdate=dt.datetime.now() - delta_day,
                                  side=tk.LEFT)

        self.end_cal = Calendar(ftwo,
                                selectmode="day",
                                mindate=tw_start_date,
        ...
        self.msg2 = tk.Label(
            ...
            font=("Times", 12),
            bg="#cae7e8")
        self.msg_display = tk.Label(self.ws, text="", bg="#cae7e8")

        #initialize combobox for interval length variable selection
        self.interval_bxstr = tk.StringVar(value=240)
        self.interval_bx = ttk.Combobox(self.ws,
                                        textvariable=self.interval_bxstr)
    ...
    def display_msg(self):
        ...
        self._totaltime = (self.end_dt -
                           self.start_dt) / dt.timedelta(minutes=1)
        ...
        if float(self._interval_len) < 1:
            raise TwitterAPIArgumentError(
                f"Invalid time interval ({self._interval_len}) received")
        pluralizer = "s" if float(self._interval_len) > 1 else ""
        if float(self._json_max) < 1:
            raise TwitterAPIArgumentError(
                f"Invalid rate of tweets ({self._json_max}) per ({self._interval_len}) minute{pluralizer} requested. At least one tweet must be requested per time interval."
            )
